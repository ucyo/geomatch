...
def create_query(center, distance_km, delta):
    ...
    {"loc":
       {"$geoWithin":
          {"$centerSphere": [ [ center.lon, center.lat ], distance_km / 6378.1 ] }
       }
    },
    {"time":
          {"$gte": lo, "$lte": up}
...
def parallel_mongo(candidates_client, num, distance_km, delta, rparams={"_id"}):
     with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = {}
        for (k,center) in tropomi[:num].iterrows():
            key = executor.submit(mongo_query, candidates_client, center, distance_km, delta, rparams)
...
def mongo_query(candidates_client, center, distance_km, delta, rparams={"_id"}):
    multiparam = create_query(center, distance_km, delta)
    result = candidates_client["IASI"].v0.find(multiparam, rparams)
