...
def parallel_process(num, distance_radius, delta):
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        for (k,center) in tropomi[:num].iterrows():
            ...
            key = executor.submit(gm.filter_candidates_by_time, center, filtered_t, delta)
...
def parallel_thread(num, distance_radius, delta):
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for (k,center) in tropomi[:num].iterrows():
            ...
            # key = executor.submit(gm.filter_by_distance, center, filtered_t, distance_threshold_m)
            futures[key]=center["_id"]
